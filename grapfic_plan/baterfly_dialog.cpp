...
#include "ui_baterfly_dialog.h"
#include <qmath.h>

Baterfly_Dialog::Baterfly_Dialog(QPointF startpoint, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Baterfly_Dialog)
{
...
connect(ui->Height_D1_edit,SIGNAL(editingFinished()),this,SLOT(changedDistance1()));
 ...
 connect(ui->geo_coord_radioB,&QRadioButton::toggled,this,&Baterfly_Dialog::coordChange);
 connect(ui->local_coord_radioB,&QRadioButton::toggled,this,&Baterfly_Dialog::coordChange);
...
void Baterfly_Dialog::mainPaiting(){
     painter->clear();
 // стилі малювання
...
// стрілки ліній  і лінії прчмокутника
   ...
   // точки в вершинах прямокутника
    ...
    // піпис точок
            textPathD.addText(PointVector[i].x() ,PointVector[i].y()-8 - i*5, timesFont,tr("ХТТ N%1").arg(i+1));
            }
...
Line_D1.lineTo(Point1.x(),Point1.y()-150);

Line_D2.moveTo(Point3.x(),Point3.y());
Line_D2.lineTo(Point3.x(),Point3.y()-150);
// малювання   лінії D1
Line_D.moveTo(Point1.x(),Point1.y()-130);
Line_D.lineTo(Point3.x(),Point3.y()-130);
...
// малювання   стілки лінії  D
  ...
  Line_D1.lineTo(Point2.x()+150,Point2.y());

  Line_D2.moveTo(Point4.x(),Point4.y());
  Line_D2.lineTo(Point4.x()+150,Point4.y());
// малювання   лінії D
  Line_D.moveTo(130, Point2.y());
  Line_D.lineTo(130, Point4.y());
...
//точки Х центру кола
void Baterfly_Dialog::changedCenterX(){
...
//точки Y центру кола
void Baterfly_Dialog::changedCenterY(){
 centerY = ui->center_y_edit->text().toDouble();
 setXY= true;

}

void Baterfly_Dialog::coordChange()
{
...
// зміна відстані 1
void Baterfly_Dialog::changedDistance1(){
    Distance1 = ui->Height_D1_edit->text().toInt();

}
 // зміна відстані 2
void Baterfly_Dialog::changedDistance2(){
    Distance2 = ui->Height_D2_edit->text().toInt();

}
// зчитування кута  з вікна
void Baterfly_Dialog::changedAngle(){
...
//
QVector<PointOfTrace> Baterfly_Dialog::getResPath(){
    QPainterPath ResPath;
   QVector<PointOfTrace> resPoints;
    //малювання вихыдноъ фыгури
    ResPath = QPainterPath();
    ResPath.moveTo(-Distance1/2 ,0);
   ResPath.lineTo(Distance1/2,0);
    ResPath.lineTo( 0,-Distance2/2);
     ResPath.lineTo(0,Distance2/2);
     ResPath.lineTo(Distance1/2,0);
     ResPath.lineTo(-Distance1/2,0);
     ResPath.lineTo(0,Distance2/2);
...
//
qreal Baterfly_Dialog::getAngle(){
    return Angle;
}
void Baterfly_Dialog::changedDirection(){
    mainPaiting();
}
Baterfly_Dialog::~Baterfly_Dialog()
{
    delete ui;
}//
bool Baterfly_Dialog::getSetXY(){
  return setXY;
}
